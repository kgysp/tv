var rule = {
	...
	class_name: '电影&电视剧&综艺&动漫',     // 筛选 /api/term/ad_fenlei?limit=10&page=1
	class_url: '1&2&4&3',
	...
	二级: `js:
		...
		try {
			...
			if (node.have_ftp_ur == 1) {
				playMap["坤哥超清"] = node.new_ftp_list.map(it => {
					return it.title + "$" + (/m3u8/.test(it.url) ? play_url + it.url : "tvbox-xg:" + it.url)
				}).join('#');
			}
			if (node.have_m3u8_ur == 1) {
				playMap["坤哥普清"] = node.new_m3u8_list.map(it => {
					return it.title + "$" + (/m3u8/.test(it.url) ? play_url + it.url : "tvbox-xg:" + it.url)
				}).join('#');
			}
            let playFrom = [];
